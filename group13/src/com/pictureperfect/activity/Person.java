...
public class Person {
	...
	private Faces getBestMatch(ArrayList<Faces> faces, Faces baseFace)
	{
		...
		Faces bestFace = faces.get(0);
		int dstWidth = baseFace.getFacePos().getWidth();
		int dstHeight = baseFace.getFacePos().getHeight();
		int minIndex = -1;
		for(int i = 0; i< faces.size();i++)
		{
			Bitmap faceTemp = Bitmap.createScaledBitmap(faces.get(i).getFaceImg(), dstWidth, dstHeight, false);
			ImageMatcher imgMatcher = new TemplateMatcher();
			int matchIndex = imgMatcher.getMatchIndex(faceTemp, baseFace.getFaceImg());
			if(i==0)
			{
				minIndex = matchIndex;
			}
